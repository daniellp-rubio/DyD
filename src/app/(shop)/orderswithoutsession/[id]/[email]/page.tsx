...
export default async function OrderWithoutSessionPage({ params, searchParams }: Props) {
  const { id, email } = await params;
  const { token } = await searchParams;

  if (!token) redirect("/");

  const result = await getOrderByIdWithoutSession(id, token);
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl shadow-xl p-7">
            ...
            <div className="grid grid-cols-2">
              ...
              <span className="text-right">
                {orderById.itemsInOrder === 1 ? "1 producto" : `${orderById.itemsInOrder} productos`}
              </span>
              <span>Subtotal</span>
            ...
            <div className="mt-5 mb-2 w-full">
              <ButtonPaid
                ...
                buyerEmail={decodeURIComponent(email)}
              />
            </div>
          </div>
        </div>
